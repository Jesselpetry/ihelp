...
// ── Blueprint drill ──────────────────────────────────────────────────────────
// Section A of data/it-kmitl/mfit/mock-exam.md: 20 rapid-fire MCQ written
// against the instructor's Q&A blueprint (10 questions / 180 min / numeric
// answers). `blueprintQ` in each sourceRef notes which real exam question the
// item drills.
export const MFIT_BLUEPRINT_QUIZ: QuizQuestion[] = [
  {
    id: "mfit-b1",
    kind: "mcq",
    chapter: 2,
    prompt: {
      th: "ข้อความ `\"IT IS FUN\"` เข้ารหัสด้วยกุญแจ 3×3 ต้องแบ่งกี่บล็อก และเติมช่องว่าง (0) กี่ตัว",
      en: "Encoding `\"IT IS FUN\"` with a 3×3 key needs how many blocks, and how many padding zeros?",
    },
    options: [
      { id: "a", label: { th: "4 บล็อก เติม 3 ตัว", en: "4 blocks, 3 padding" }, why: { th: "ผิด — 9 อักขระพอดี 3 บล็อก ไม่ต้องขึ้นบล็อกที่ 4", en: "Wrong — 9 characters fill exactly 3 blocks; no fourth block is needed." } },
      { id: "b", label: { th: "3 บล็อก เติม 2 ตัว", en: "3 blocks, 2 padding" }, why: { th: "ผิด — กับดักที่ตั้งใจวางไว้ คือลืมนับช่องว่างเป็นอักขระ (นับได้แค่ ITISFUN = 7) **ช่องว่างคืออักขระตัวหนึ่ง แทนด้วยเลข 0**", en: "Wrong — the deliberate trap: forgetting spaces are characters gives just ITISFUN = 7. A space is a character, encoded as 0." } },
      { id: "c", label: { th: "3 บล็อก เติม 0 ตัว", en: "3 blocks, 0 padding" }, why: { th: "ถูกต้อง — `I,T,␣,I,S,␣,F,U,N` = 9 อักขระ (นับช่องว่าง 2 ตัวด้วย) → 9 ÷ 3 = 3 พอดี", en: "Correct — `I,T,␣,I,S,␣,F,U,N` is 9 characters counting both spaces, and 9 ÷ 3 = 3 exactly." } },
      { id: "d", label: { th: "3 บล็อก เติม 1 ตัว", en: "3 blocks, 1 padding" }, why: { th: "ผิด — นับอักขระได้ 8 ตัว ยังตกช่องว่างไป 1 ตัว", en: "Wrong — that counts 8 characters, missing one space." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 1 · blueprintQ 1-2",
  },
  {
    id: "mfit-b2",
    kind: "mcq",
    chapter: 2,
    prompt: { th: "สูตรถอดรหัสที่ถูกต้องคือข้อใด", en: "Which is the correct decoding formula?" },
    options: [
      { id: "a", label: { th: "`X = A⁻¹Y`", en: "`X = A⁻¹Y`" }, why: { th: "ผิด — คูณผิดด้าน การคูณเมทริกซ์ไม่สลับที่", en: "Wrong — wrong side; matrix multiplication does not commute." } },
      { id: "b", label: { th: "`X = AY`", en: "`X = AY`" }, why: { th: "ผิด — คูณด้วย A ซ้ำ ยิ่งเข้ารหัสหนักเข้าไปอีก", en: "Wrong — multiplying by A again encodes further rather than decoding." } },
      { id: "c", label: { th: "`X = YA`", en: "`X = YA`" }, why: { th: "ผิด — ต้องใช้ตัวผกผัน ไม่ใช่ A ตัวเดิม", en: "Wrong — decoding needs the inverse, not A itself." } },
      { id: "d", label: { th: "`X = YA⁻¹`", en: "`X = YA⁻¹`" }, why: { th: "ถูกต้อง — เข้ารหัสคือ `Y = XA` จึงคูณ `A⁻¹` เข้าทาง **ขวา** เพื่อย้อนกลับ", en: "Correct — encoding is `Y = XA`, so you right-multiply by `A⁻¹` to undo it." } },
    ],
    correctId: "d",
    sourceRef: "mock-exam.md Section A ข้อ 2 · blueprintQ 1-2",
  },
  {
    id: "mfit-b3",
    kind: "mcq",
    chapter: 2,
    prompt: { th: "เหตุใดกุญแจ `A` ต้องหาตัวผกผันได้", en: "Why must the key `A` be invertible?" },
    options: [
      { id: "a", label: { th: "เพื่อให้บล็อกครบ", en: "To make the blocks come out even" }, why: { th: "ผิด — การแบ่งบล็อกขึ้นกับขนาด n ของกุญแจ ไม่ใช่ det", en: "Wrong — blocking depends on the key's size n, not its determinant." } },
      { id: "b", label: { th: "เพื่อให้ det = 1", en: "To force det = 1" }, why: { th: "ผิด — `det = ±1` เป็นเพียงความสะดวกให้ได้จำนวนเต็ม ไม่ใช่ข้อบังคับ", en: "Wrong — `det = ±1` is a convenience for integer results, not a requirement." } },
      { id: "c", label: { th: "เพื่อให้ถอดรหัสกลับได้", en: "So the message can be decoded" }, why: { th: "ถูกต้อง — ถ้า `det(A) = 0` จะไม่มี `A⁻¹` และข้อความคนละอันอาจเข้ารหัสได้ผลเดียวกัน ถอดกลับไม่ได้", en: "Correct — if `det(A) = 0` there is no `A⁻¹`, and different messages can encode identically, making decoding impossible." } },
      { id: "d", label: { th: "เพื่อให้ตัวเลขไม่ติดลบ", en: "To keep the numbers non-negative" }, why: { th: "ผิด — cryptogram มีเลขติดลบได้ตามปกติ", en: "Wrong — cryptograms routinely contain negative numbers." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 3 · blueprintQ 1-2",
  },
  {
    id: "mfit-b4",
    kind: "mcq",
    chapter: 2,
    prompt: { th: "`adj(A)` ได้มาจากอะไร", en: "How is `adj(A)` obtained?" },
    options: [
      { id: "a", label: { th: "เมทริกซ์โคแฟกเตอร์ (ไม่ต้อง transpose)", en: "The cofactor matrix, untransposed" }, why: { th: "ผิด — ขาดขั้น transpose ทำให้ `A⁻¹` ผิดทั้งตัวและถอดรหัสไม่ออก", en: "Wrong — skipping the transpose corrupts all of `A⁻¹` and the decode fails." } },
      { id: "b", label: { th: "`A` transpose", en: "`A` transposed" }, why: { th: "ผิด — ต้องคำนวณโคแฟกเตอร์ก่อน ไม่ใช่ transpose ตัว A ตรงๆ", en: "Wrong — you must compute cofactors first, not transpose A itself." } },
      { id: "c", label: { th: "`A⁻¹ × det(A)ᵀ`", en: "`A⁻¹ × det(A)ᵀ`" }, why: { th: "ผิด — det เป็นสเกลาร์ transpose ไม่ได้ (แม้ `adj(A) = A⁻¹·det(A)` จะจริง แต่ใช้หา A⁻¹ ไม่ได้เพราะวนกลับ)", en: "Wrong — det is a scalar and cannot be transposed. (`adj(A) = A⁻¹·det(A)` is true but circular when A⁻¹ is what you want.)" } },
      { id: "d", label: { th: "เมทริกซ์โคแฟกเตอร์แล้ว transpose", en: "The cofactor matrix, then transposed" }, why: { th: "ถูกต้อง — `adj(A) = [Cᵢⱼ]ᵀ` **ลืม transpose คือกับดักอันดับ 1 ของข้อสอบชุดนี้**", en: "Correct — `adj(A) = [Cᵢⱼ]ᵀ`. Forgetting the transpose is the single biggest score-loser on this paper." } },
    ],
    correctId: "d",
    sourceRef: "mock-exam.md Section A ข้อ 4 · blueprintQ 1-2",
  },
  {
    id: "mfit-b5",
    kind: "mcq",
    chapter: 3,
    prompt: { th: "ใช้ Cramer's Rule หา `x₂` ต้องคำนวณอย่างไร", en: "Using Cramer's Rule, how do you compute `x₂`?" },
    options: [
      { id: "a", label: { th: "`det(A₂)/det(A)` โดย `A₂` แทน **แถวที่ 2** ด้วย `b`", en: "`det(A₂)/det(A)`, replacing **row 2** with `b`" }, why: { th: "ผิด — กับดักหลักของข้อนี้ แทนแถวจะได้คำตอบผิดทันที", en: "Wrong — the main trap here; replacing a row gives an immediately wrong answer." } },
      { id: "b", label: { th: "`det(A)/det(A₂)`", en: "`det(A)/det(A₂)`" }, why: { th: "ผิด — กลับเศษส่วน ตัวส่วนต้องเป็น `det(A)` เสมอ", en: "Wrong — inverted; the denominator is always `det(A)`." } },
      { id: "c", label: { th: "`det(A₂) − det(A)`", en: "`det(A₂) − det(A)`" }, why: { th: "ผิด — เป็นการหาร ไม่ใช่การลบ", en: "Wrong — the rule divides, it does not subtract." } },
      { id: "d", label: { th: "`det(A₂)/det(A)` โดย `A₂` แทน **คอลัมน์ที่ 2** ด้วย `b`", en: "`det(A₂)/det(A)`, replacing **column 2** with `b`" }, why: { th: "ถูกต้อง — แทนที่คอลัมน์เสมอ เพราะตัวแปรแต่ละตัวคูณอยู่กับคอลัมน์ของมัน", en: "Correct — always replace a column, since each unknown multiplies its own column." } },
    ],
    correctId: "d",
    sourceRef: "mock-exam.md Section A ข้อ 5 · blueprintQ 3",
  },
  {
    id: "mfit-b6",
    kind: "mcq",
    chapter: 3,
    prompt: { th: "ระบบสมการที่มี `det(A) = 0` ควรใช้วิธีใด", en: "Which method should you use when `det(A) = 0`?" },
    options: [
      { id: "a", label: { th: "`x = A⁻¹b`", en: "`x = A⁻¹b`" }, why: { th: "ผิด — `det = 0` แปลว่าไม่มี `A⁻¹`", en: "Wrong — `det = 0` means `A⁻¹` does not exist." } },
      { id: "b", label: { th: "ไม่ต้องทำ เพราะไม่มีผลเฉลย", en: "Skip it — there is no solution" }, why: { th: "ผิด — `det = 0` อาจหมายถึงผลเฉลย **อนันต์** ก็ได้ ต้องตรวจก่อน", en: "Wrong — `det = 0` may equally mean infinitely many solutions; you must check." } },
      { id: "c", label: { th: "Gauss-Jordan", en: "Gauss-Jordan" }, why: { th: "ถูกต้อง — เป็นวิธีเดียวที่ยังใช้ได้ และบอกได้ด้วยว่าไม่มีผลเฉลยหรือมีอนันต์", en: "Correct — the only method that still works, and it also reveals whether there are no solutions or infinitely many." } },
      { id: "d", label: { th: "Cramer's Rule", en: "Cramer's Rule" }, why: { th: "ผิด — ต้องหารด้วย `det(A)` ซึ่งเป็นศูนย์", en: "Wrong — it divides by `det(A)`, which is zero." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 6 · blueprintQ 3",
  },
  {
    id: "mfit-b7",
    kind: "mcq",
    chapter: 3,
    prompt: { th: "หลังทำ Gauss-Jordan ได้แถว `[0 0 0 | 7]` แสดงว่าอะไร", en: "After Gauss-Jordan you get the row `[0 0 0 | 7]`. What does it mean?" },
    options: [
      { id: "a", label: { th: "`x₃ = 7`", en: "`x₃ = 7`" }, why: { th: "ผิด — สัมประสิทธิ์ของ `x₃` เป็น 0 จึงอ่านค่าแบบนั้นไม่ได้", en: "Wrong — the coefficient of `x₃` is 0, so it cannot be read that way." } },
      { id: "b", label: { th: "ไม่มีผลเฉลย (inconsistent)", en: "No solution — inconsistent" }, why: { th: "ถูกต้อง — แถวนี้อ่านว่า `0 = 7` ซึ่งเป็นเท็จ ถ้าเป็น `[0 0 0 | 0]` ถึงจะเป็นผลเฉลยอนันต์", en: "Correct — the row reads `0 = 7`, which is false. `[0 0 0 | 0]` would instead signal infinitely many solutions." } },
      { id: "c", label: { th: "ผลเฉลยอนันต์", en: "Infinitely many solutions" }, why: { th: "ผิด — ต้องเป็นแถวศูนย์ล้วนรวมพจน์คงที่ด้วย", en: "Wrong — that needs the constant term to be zero as well." } },
      { id: "d", label: { th: "ผลเฉลยเดียว", en: "A unique solution" }, why: { th: "ผิด — ต้องได้เมทริกซ์เอกลักษณ์ทางซ้ายจึงจะมีผลเฉลยเดียว", en: "Wrong — a unique solution requires the identity on the left." } },
    ],
    correctId: "b",
    sourceRef: "mock-exam.md Section A ข้อ 7 · blueprintQ 3",
  },
  {
    id: "mfit-b8",
    kind: "mcq",
    chapter: 4,
    prompt: { th: "`‖(3, −4, 12)‖` เท่ากับเท่าใด", en: "What is `‖(3, −4, 12)‖`?" },
    options: [
      { id: "a", label: { th: "11", en: "11" }, why: { th: "ผิด — บวก 3 − 4 + 12 ตรงๆ โดยไม่ยกกำลังสอง", en: "Wrong — that just adds 3 − 4 + 12 without squaring." } },
      { id: "b", label: { th: "19", en: "19" }, why: { th: "ผิด — บวกค่าสัมบูรณ์ 3 + 4 + 12", en: "Wrong — that sums the absolute values 3 + 4 + 12." } },
      { id: "c", label: { th: "13", en: "13" }, why: { th: "ถูกต้อง — `√(9 + 16 + 144) = √169 = 13`", en: "Correct — `√(9 + 16 + 144) = √169 = 13`." } },
      { id: "d", label: { th: "169", en: "169" }, why: { th: "ผิด — ลืมถอดรากที่สอง ค่านี้คือ `‖u‖²`", en: "Wrong — the square root was skipped; that value is `‖u‖²`." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 8 · blueprintQ 4",
  },
  {
    id: "mfit-b9",
    kind: "mcq",
    chapter: 4,
    prompt: { th: "`u = (2, −3, 1)`, `v = (4, 3, 1)` มุมระหว่างเวกเตอร์คือเท่าใด", en: "For `u = (2, −3, 1)` and `v = (4, 3, 1)`, what is the angle between them?" },
    options: [
      { id: "a", label: { th: "90°", en: "90°" }, why: { th: "ถูกต้อง — `2(4) + (−3)(3) + 1(1) = 8 − 9 + 1 = 0` → ตั้งฉาก ไม่ต้องคำนวณ norm เลย", en: "Correct — `2(4) + (−3)(3) + 1(1) = 8 − 9 + 1 = 0`, so they are orthogonal. No norms needed." } },
      { id: "b", label: { th: "0°", en: "0°" }, why: { th: "ผิด — ต้องเป็นพหุคูณกัน ซึ่งไม่ใช่", en: "Wrong — that requires the vectors to be parallel, which they are not." } },
      { id: "c", label: { th: "45°", en: "45°" }, why: { th: "ผิด — `cos 45° ≠ 0` แต่ dot product เป็นศูนย์", en: "Wrong — `cos 45° ≠ 0`, yet the dot product is zero." } },
      { id: "d", label: { th: "180°", en: "180°" }, why: { th: "ผิด — ต้องชี้ตรงข้าม ซึ่งให้ dot product เป็นลบ", en: "Wrong — that needs opposite directions and a negative dot product." } },
    ],
    correctId: "a",
    sourceRef: "mock-exam.md Section A ข้อ 9 · blueprintQ 4",
  },
  {
    id: "mfit-b10",
    kind: "mcq",
    chapter: 4,
    prompt: { th: "ถ้า `u · v < 0` แสดงว่ามุม θ เป็นอย่างไร", en: "If `u · v < 0`, what kind of angle is θ?" },
    options: [
      { id: "a", label: { th: "เป็นมุมแหลม", en: "Acute" }, why: { th: "ผิด — มุมแหลมให้ dot product เป็นบวก", en: "Wrong — an acute angle gives a positive dot product." } },
      { id: "b", label: { th: "เป็น 90°", en: "Exactly 90°" }, why: { th: "ผิด — 90° ให้ dot product เป็นศูนย์พอดี", en: "Wrong — 90° gives exactly zero." } },
      { id: "c", label: { th: "หาไม่ได้", en: "Undefined" }, why: { th: "ผิด — หาได้เสมอตราบใดที่ทั้งสองเวกเตอร์ไม่เป็นศูนย์", en: "Wrong — it is always defined as long as neither vector is zero." } },
      { id: "d", label: { th: "เป็นมุมป้าน (90° < θ ≤ 180°)", en: "Obtuse — 90° < θ ≤ 180°" }, why: { th: "ถูกต้อง — `cos θ` มีเครื่องหมายเดียวกับ dot product ดังนั้นลบ → มุมป้าน", en: "Correct — `cos θ` carries the same sign as the dot product, so negative means obtuse." } },
    ],
    correctId: "d",
    sourceRef: "mock-exam.md Section A ข้อ 10 · blueprintQ 4",
  },
  {
    id: "mfit-b11",
    kind: "mcq",
    chapter: 4,
    prompt: { th: "`u · u` เท่ากับข้อใด", en: "What does `u · u` equal?" },
    options: [
      { id: "a", label: { th: "`2‖u‖`", en: "`2‖u‖`" }, why: { th: "ผิด — ไม่มีกฎนี้", en: "Wrong — no such identity." } },
      { id: "b", label: { th: "`0`", en: "`0`" }, why: { th: "ผิด — เป็นศูนย์เฉพาะเมื่อ `u` เป็นเวกเตอร์ศูนย์", en: "Wrong — zero only when `u` is the zero vector." } },
      { id: "c", label: { th: "`‖u‖²`", en: "`‖u‖²`" }, why: { th: "ถูกต้อง — `u·u = u₁² + u₂² + … = ‖u‖²`", en: "Correct — `u·u = u₁² + u₂² + … = ‖u‖²`." } },
      { id: "d", label: { th: "`‖u‖`", en: "`‖u‖`" }, why: { th: "ผิด — กับดักคลาสสิก norm คือรากที่สองของ `u·u`", en: "Wrong — the classic trap; the norm is the square root of `u·u`." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 11 · blueprintQ 4",
  },
  {
    id: "mfit-b12",
    kind: "mcq",
    chapter: 5,
    prompt: { th: "เซตใด **ไม่เป็น** ปริภูมิเวกเตอร์", en: "Which set is **not** a vector space?" },
    options: [
      { id: "a", label: { th: "`M₂,₃`", en: "`M₂,₃`" }, why: { th: "ผิด — เมทริกซ์ขนาดคงที่ทั้งหมดเป็น vector space", en: "Wrong — the set of all matrices of a fixed size is a vector space." } },
      { id: "b", label: { th: "`R⁵`", en: "`R⁵`" }, why: { th: "ผิด — `Rⁿ` เป็นตัวอย่างมาตรฐานที่สุด", en: "Wrong — `Rⁿ` is the canonical example." } },
      { id: "c", label: { th: "เซตของพหุนามดีกรีเท่ากับ 3 พอดี", en: "Polynomials of degree exactly 3" }, why: { th: "ถูกต้อง — ไม่ปิดใต้การบวก (`x³ + (−x³+1) = 1`) และไม่มีพหุนามศูนย์ ต้องเป็น \"ดีกรีไม่เกิน\" ถึงจะใช่", en: "Correct — not closed under addition (`x³ + (−x³+1) = 1`) and it has no zero polynomial. \"Degree at most\" would be fine." } },
      { id: "d", label: { th: "`P₃` (พหุนามดีกรีไม่เกิน 3)", en: "`P₃` — polynomials of degree at most 3" }, why: { th: "ผิด — เป็น vector space มาตรฐาน", en: "Wrong — a standard vector space." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 12 · blueprintQ 5-6",
  },
  {
    id: "mfit-b13",
    kind: "mcq",
    chapter: 5,
    prompt: {
      th: "`V = {(x, y) : x ≥ 0, y ≥ 0}` ไม่เป็นปริภูมิเวกเตอร์เพราะพังสัจพจน์ข้อใด",
      en: "`V = {(x, y) : x ≥ 0, y ≥ 0}` fails which axiom?",
    },
    options: [
      { id: "a", label: { th: "ข้อ 10 — `1·u = u`", en: "Axiom 10 — `1·u = u`" }, why: { th: "ผิด — คูณด้วย 1 ไม่เปลี่ยนอะไร จึงยังอยู่ในเซต", en: "Wrong — multiplying by 1 changes nothing, so the element stays put." } },
      { id: "b", label: { th: "ข้อ 6 — ปิดใต้การคูณสเกลาร์", en: "Axiom 6 — closure under scalar multiplication" }, why: { th: "ถูกต้อง — บวกกันยังอยู่ควอดรันต์ 1 (ข้อ 1 ผ่าน) แต่คูณ `−1` แล้วหลุดออกนอกเซต", en: "Correct — sums stay in quadrant I so closure under addition holds, but multiplying by `−1` leaves the set." } },
      { id: "c", label: { th: "ข้อ 1 — ปิดใต้การบวก", en: "Axiom 1 — closure under addition" }, why: { th: "ผิด — บวกสองเวกเตอร์ที่มีองค์ประกอบไม่ติดลบ ยังได้องค์ประกอบไม่ติดลบ", en: "Wrong — adding two non-negative vectors keeps every component non-negative." } },
      { id: "d", label: { th: "ข้อ 2 — สลับที่", en: "Axiom 2 — commutativity" }, why: { th: "ผิด — การบวกใน `R²` สลับที่ได้เสมอ", en: "Wrong — addition in `R²` always commutes." } },
    ],
    correctId: "b",
    sourceRef: "mock-exam.md Section A ข้อ 13 · blueprintQ 5-6",
  },
  {
    id: "mfit-b14",
    kind: "mcq",
    chapter: 5,
    prompt: {
      th: "เซต `S` เป็นอิสระเชิงเส้น เมื่อสมการ `c₁v₁ + c₂v₂ + c₃v₃ = 0` มีผลเฉลยแบบใด",
      en: "`S` is linearly independent when `c₁v₁ + c₂v₂ + c₃v₃ = 0` has what kind of solution?",
    },
    options: [
      { id: "a", label: { th: "มีผลเฉลยอนันต์", en: "Infinitely many solutions" }, why: { th: "ผิด — มี free variable แปลว่า dependent", en: "Wrong — a free variable means dependent." } },
      { id: "b", label: { th: "ไม่มีผลเฉลย", en: "No solution" }, why: { th: "ผิด — สมการนี้มีผลเฉลย `c = 0` เสมอ เป็นระบบ homogeneous", en: "Wrong — being homogeneous, it always admits `c = 0`." } },
      { id: "c", label: { th: "`c₁ = c₂ = c₃ = 0` เท่านั้น", en: "Only `c₁ = c₂ = c₃ = 0`" }, why: { th: "ถูกต้อง — เป็นนิยามตรงๆ มีเฉพาะผลเฉลยชัด (trivial)", en: "Correct — that is the definition: only the trivial solution." } },
      { id: "d", label: { th: "อย่างน้อยหนึ่งตัวไม่เป็นศูนย์", en: "At least one coefficient nonzero" }, why: { th: "ผิด — นั่นคือนิยามของ **dependent**", en: "Wrong — that is the definition of dependent." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 14 · blueprintQ 7",
  },
  {
    id: "mfit-b15",
    kind: "mcq",
    chapter: 5,
    prompt: { th: "เซตของเวกเตอร์ 4 ตัวใน `R³` จะเป็นอย่างไร", en: "A set of 4 vectors in `R³` is what?" },
    options: [
      { id: "a", label: { th: "เป็น spanning set เสมอ", en: "Always a spanning set" }, why: { th: "ผิด — เช่น 4 เวกเตอร์ที่อยู่บนระนาบเดียวกัน span ได้แค่ระนาบ ไม่ใช่ทั้ง `R³`", en: "Wrong — four coplanar vectors span only that plane, not all of `R³`." } },
      { id: "b", label: { th: "ขึ้นต่อกันเชิงเส้นเสมอ", en: "Always linearly dependent" }, why: { th: "ถูกต้อง — จำนวนเวกเตอร์ > มิติของสเปซ → dependent เสมอ ตอบได้ทันทีไม่ต้องคำนวณ", en: "Correct — more vectors than dimensions forces dependence. Answer instantly, no computation." } },
      { id: "c", label: { th: "อิสระเชิงเส้นเสมอ", en: "Always linearly independent" }, why: { th: "ผิด — เป็นไปไม่ได้ `R³` รองรับเวกเตอร์อิสระได้มากสุด 3 ตัว", en: "Wrong — impossible; `R³` supports at most 3 independent vectors." } },
      { id: "d", label: { th: "สรุปไม่ได้ ต้องคำนวณก่อน", en: "Cannot tell without computing" }, why: { th: "ผิด — ทฤษฎีบทให้ข้อสรุปทันทีจากการนับ", en: "Wrong — the counting theorem settles it immediately." } },
    ],
    correctId: "b",
    sourceRef: "mock-exam.md Section A ข้อ 15 · blueprintQ 7",
  },
  {
    id: "mfit-b16",
    kind: "mcq",
    chapter: 5,
    prompt: {
      th: "เวกเตอร์ 3 ตัวใน `R³` วางเป็นคอลัมน์ได้เมทริกซ์ที่มี `det = 0` แสดงว่าอะไร",
      en: "Three vectors in `R³` form a matrix with `det = 0`. What follows?",
    },
    options: [
      { id: "a", label: { th: "ขึ้นต่อกันเชิงเส้น (dependent)", en: "Linearly dependent" }, why: { th: "ถูกต้อง — `det = 0` → ระบบมีผลเฉลยไม่ชัด → dependent (คนสลับกับ \"อิสระ\" บ่อยมาก)", en: "Correct — `det = 0` means a non-trivial solution exists, hence dependent. People flip this constantly." } },
      { id: "b", label: { th: "อิสระเชิงเส้น", en: "Linearly independent" }, why: { th: "ผิด — กลับด้าน `det ≠ 0` ต่างหากที่แปลว่าอิสระ", en: "Wrong — backwards; `det ≠ 0` means independent." } },
      { id: "c", label: { th: "เป็นฐาน (basis) ของ `R³`", en: "A basis of `R³`" }, why: { th: "ผิด — ฐานต้องอิสระเชิงเส้น จึงต้อง `det ≠ 0`", en: "Wrong — a basis must be independent, requiring `det ≠ 0`." } },
      { id: "d", label: { th: "span ได้ทั้ง `R³`", en: "They span all of `R³`" }, why: { th: "ผิด — 3 เวกเตอร์ที่ dependent span ได้แค่ระนาบหรือเส้นตรง", en: "Wrong — three dependent vectors span at most a plane or a line." } },
    ],
    correctId: "a",
    sourceRef: "mock-exam.md Section A ข้อ 16 · blueprintQ 7",
  },
  {
    id: "mfit-b17",
    kind: "mcq",
    chapter: 6,
    prompt: { th: "`T(v) = Av` โดย `A` มีขนาด 3 × 5 ดังนั้น T เป็นการแปลงแบบใด", en: "`T(v) = Av` where `A` is 3 × 5. What is T?" },
    options: [
      { id: "a", label: { th: "`T: R⁵ → R³`", en: "`T: R⁵ → R³`" }, why: { th: "ถูกต้อง — คอลัมน์ 5 = มิติต้นทาง, แถว 3 = มิติปลายทาง เพราะ `Av` ต้องคูณกันได้", en: "Correct — 5 columns give the domain, 3 rows the codomain, since `Av` must be a valid product." } },
      { id: "b", label: { th: "`T: R³ → R⁵`", en: "`T: R³ → R⁵`" }, why: { th: "ผิด — สลับ n กับ m เป็นกับดักที่เจอบ่อยที่สุดของข้อ 8", en: "Wrong — swapping n and m, the most common slip on blueprint question 8." } },
      { id: "c", label: { th: "`T: R³ → R³`", en: "`T: R³ → R³`" }, why: { th: "ผิด — จริงเฉพาะเมื่อ A จัตุรัส", en: "Wrong — only true for a square A." } },
      { id: "d", label: { th: "`T: R⁵ → R⁵`", en: "`T: R⁵ → R⁵`" }, why: { th: "ผิด — เช่นเดียวกัน ต้องจัตุรัสจึงจะเป็นเช่นนั้น", en: "Wrong — likewise requires a square matrix." } },
    ],
    correctId: "a",
    sourceRef: "mock-exam.md Section A ข้อ 17 · blueprintQ 8",
  },
  {
    id: "mfit-b18",
    kind: "mcq",
    chapter: 6,
    prompt: { th: "เมทริกซ์มาตรฐานของ `T(x, y, z) = (5z − y, 2x)` คือข้อใด", en: "What is the standard matrix of `T(x, y, z) = (5z − y, 2x)`?" },
    snippet: "จัดเรียงใหม่: (0x − 1y + 5z, 2x + 0y + 0z)",
    options: [
      { id: "a", label: { th: "`[5 −1 0; 0 0 2]`", en: "`[5 −1 0; 0 0 2]`" }, why: { th: "ผิด — สลับตำแหน่ง x กับ z", en: "Wrong — the x and z positions are swapped." } },
      { id: "b", label: { th: "`[0 2; −1 0; 5 0]`", en: "`[0 2; −1 0; 5 0]`" }, why: { th: "ผิด — คือ transpose ของคำตอบที่ถูก ขนาดต้องเป็น 2 × 3 (`T: R³ → R²`)", en: "Wrong — that is the transpose; the matrix must be 2 × 3 for `T: R³ → R²`." } },
      { id: "c", label: { th: "`[0 −1 5; 2 0 0]`", en: "`[0 −1 5; 2 0 0]`" }, why: { th: "ถูกต้อง — จัดเรียงตามลำดับ x, y, z ก่อนเสมอ แล้วเติม 0 ให้ตัวแปรที่หายไป", en: "Correct — reorder into x, y, z first, then fill 0 for every absent variable." } },
      { id: "d", label: { th: "`[−1 5 0; 2 0 0]`", en: "`[−1 5 0; 2 0 0]`" }, why: { th: "ผิด — เรียงตามที่โจทย์เขียน (`5z − y`) โดยไม่จัดใหม่ **ไม่เติม 0 ให้ตัวแปรที่หายไป = จุดเสียคะแนนอันดับ 3**", en: "Wrong — copies the written order (`5z − y`) without reordering. Failing to slot in the missing variable is a top-three score-loser." } },
    ],
    correctId: "c",
    sourceRef: "mock-exam.md Section A ข้อ 18 · blueprintQ 8",
  },
  {
    id: "mfit-b19",
    kind: "mcq",
    chapter: 6,
    prompt: { th: "เมทริกซ์มาตรฐานของ `T = T₂ ∘ T₁` คือข้อใด", en: "What is the standard matrix of `T = T₂ ∘ T₁`?" },
    options: [
      { id: "a", label: { th: "`A₂A₁`", en: "`A₂A₁`" }, why: { th: "ถูกต้อง — ทำ `T₁` ก่อน แต่เมทริกซ์ของตัวที่ทำทีหลังอยู่ **ซ้าย**", en: "Correct — `T₁` acts first, but the later transformation's matrix sits on the left." } },
      { id: "b", label: { th: "`A₁A₂`", en: "`A₁A₂`" }, why: { th: "ผิด — เรียงตามลำดับที่เขียน แต่การประกอบทำงานจากในออกนอก", en: "Wrong — follows the written order, but composition works inside-out." } },
      { id: "c", label: { th: "`A₁ + A₂`", en: "`A₁ + A₂`" }, why: { th: "ผิด — การประกอบคือการคูณ ไม่ใช่การบวก", en: "Wrong — composition is multiplication, not addition." } },
      { id: "d", label: { th: "`A₁A₂ᵀ`", en: "`A₁A₂ᵀ`" }, why: { th: "ผิด — ไม่มี transpose เข้ามาเกี่ยวข้อง", en: "Wrong — no transpose is involved." } },
    ],
    correctId: "a",
    sourceRef: "mock-exam.md Section A ข้อ 19 · blueprintQ 8",
  },
  {
    id: "mfit-b20",
    kind: "mcq",
    chapter: 7,
    prompt: {
      th: "ค่าเจาะจงของ `A = [6 4 −1; 0 −2 3; 0 0 5]` คือข้อใด",
      en: "What are the eigenvalues of `A = [6 4 −1; 0 −2 3; 0 0 5]`?",
    },
    options: [
      { id: "a", label: { th: "6, −2, 5", en: "6, −2, 5" }, why: { th: "ถูกต้อง — เมทริกซ์สามเหลี่ยมบน อ่านแนวทแยงหลักได้เลย ตรวจ: `6 − 2 + 5 = 9 = trace` ✓", en: "Correct — upper triangular, so read the diagonal. Check: `6 − 2 + 5 = 9 = trace` ✓." } },
      { id: "b", label: { th: "6, 4, −1", en: "6, 4, −1" }, why: { th: "ผิด — อ่านแถวแรกแทนแนวทแยง", en: "Wrong — that reads the first row instead of the diagonal." } },
      { id: "c", label: { th: "ต้องแก้สมการดีกรี 3 ก่อน", en: "You must solve a cubic first" }, why: { th: "ผิด — เสียเวลา 15 นาทีฟรี ทางลัดนี้ต้องจับให้ได้ใน 5 วินาที", en: "Wrong — that wastes 15 minutes; this shortcut should register within 5 seconds." } },
      { id: "d", label: { th: "9", en: "9" }, why: { th: "ผิด — 9 คือ trace (ผลรวมของ eigenvalue) ไม่ใช่ตัว eigenvalue", en: "Wrong — 9 is the trace, the sum of the eigenvalues, not an eigenvalue." } },
    ],
    correctId: "a",
    sourceRef: "mock-exam.md Section A ข้อ 20 · blueprintQ 10",
  },
];
